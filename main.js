...
function loadServerConfig() {
  ...
  catch { cfg = { url: DEFAULT_RTHUB_URL, autoConnect: false, sessionId: '' }; }
  if (typeof cfg.url !== 'string' || !cfg.url) cfg.url = DEFAULT_RTHUB_URL;
  if (typeof cfg.sessionId !== 'string') cfg.sessionId = '';
  if (typeof cfg.autoConnect !== 'boolean') cfg.autoConnect = false;
  // One-shot migration off the dead local server.
  if (/^ws:\/\/localhost:3535/.test(cfg.url)) cfg.url = DEFAULT_RTHUB_URL;
  return cfg;
}
