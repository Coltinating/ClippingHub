    ...
    <div class="config-modal wm-modal" style="width:520px; max-height:85vh; overflow-y:auto;">
      ...
      <div class="wm-modal-body" style="gap:16px;">
        ...
        <div class="config-section" data-settings-tab="developer" style="border:1px dashed #ef4444; border-radius:6px; padding:10px; margin-top:8px;">
          ...
          <label class="config-toggle"><input type="checkbox" id="cfgBatchEnabled" ${batchModeEnabled?'checked':''}> <span class="config-toggle-label">Enable Batch Testing Mode</span> <span class="config-help" tabindex="0" data-tip="Press B to toggle batch mode. Creates N identical clips from a single IN/OUT for encoding comparison. Each batch outputs to a subfolder named after the ffmpeg config, with a manifest .txt documenting every command used.">?</span></label>
          <label class="config-toggle" style="margin-top:6px;"><input type="checkbox" id="cfgFfmpegLogs" ${cfg.devFeatures?.ffmpegLogs?'checked':''}> <span class="config-toggle-label">Show &quot;View FFMPEG Log&quot; on completed clips</span> <span class="config-help" tabindex="0" data-tip="Adds a per-clip FFMPEG Log entry to the More menu on completed downloads. Opens a debug window with the exact ffmpeg commands and stderr captured during that clip's encode.">?</span></label>
          <label class="config-toggle" style="margin-top:6px;"><input type="checkbox" id="cfgKeepTempFiles" ${cfg.devFeatures?.keepTempFiles?'checked':''}> <span class="config-toggle-label">Keep temp files after clip download</span> <span class="config-help" tabindex="0" data-tip="Skips cleanup of intermediate .ts segments, concat lists, and watermark stages. Useful when an encode produces wrong output and you want to inspect the inputs.">?</span></label>
          <label class="config-toggle" style="margin-top:6px;"><input type="checkbox" id="cfgLogFfmpegCommands" ${cfg.devFeatures?.logFfmpegCommands?'checked':''}> <span class="config-toggle-label">Output all FFmpeg commands to debug log</span> <span class="config-help" tabindex="0" data-tip="Mirrors every ffmpeg invocation (with full args) into the debug log pane and the per-clip log buffer. High-volume — leave off for normal use.">?</span></label>
          <label class="config-toggle" style="margin-top:6px;"><input type="checkbox" id="cfgAdvancedPanels" ${cfg.devFeatures?.advancedPanelSystem?'checked':''}> <span class="config-toggle-label">Enable advanced panel system</span> <span class="config-help" tabindex="0" data-tip="Unlocks drag, split, dock, undock, and custom workspace layouts. Off keeps a simpler fixed layout that's friendlier for new users.">?</span></label>
          <label class="config-toggle" style="margin-top:6px;"><input type="checkbox" id="cfgFrameAccurateClipping" ${cfg.devFeatures?.frameAccurateClipping?'checked':''}> <span class="config-toggle-label">Frame-accurate clipping</span> <span class="config-help" tabindex="0" data-tip="Experimental. Forces IN/OUT trims to land on the exact frame the user marked instead of the nearest GOP boundary. Off keeps the main encode pipeline untouched.">?</span></label>
          <label class="config-toggle" style="margin-top:6px;"><input type="checkbox" id="cfgTutorialDev" ${tutorialDevEnabled?'checked':''}> <span class="config-toggle-label">Tutorial system</span> <span class="config-help" tabindex="0" data-tip="In-development guided walkthrough of the app. Toggling reloads the window so the overlay engine can attach. State is not persisted to user config.">?</span></label>
          <label class="config-toggle" style="margin-top:6px;"><input type="checkbox" id="cfgShazamScan" ${cfg.devFeatures?.shazamScan?'checked':''}> <span class="config-toggle-label">Shazam button on pending clips</span> <span class="config-help" tabindex="0" data-tip="Experimental. Adds a Shazam button on each pending clip that scans its IN→OUT range for music and appends recognized tracks to the configured .txt file. Requires the music/.venv with shazamio installed.">?</span></label>
